...
class Phase2PixelRawNtuple : public edm::one::EDAnalyzer<> {
public:
  explicit Phase2PixelRawNtuple(const edm::ParameterSet& conf);
  virtual ~Phase2PixelRawNtuple();
...
void Phase2PixelRawNtuple::analyze(const edm::Event& e, const edm::EventSetup& es) {

  cout << "In Phase2PixelRawNtuple::analyze" << endl;
  ...
  for ( iterDet = aQCoreVector->begin();
    ...
    for ( auto iterQCore = theQCores.begin();
          ...
          ++iterQCore ) {
      std::cout << "QCORE : " << iterQCore->rocid() << " " << iterQCore->get_col() << " " << iterQCore->get_row() << std::endl;
    }
  }
...
void Phase2PixelRawNtuple::fillPRecHit(const int detid_db,
                                    const int subid,
                                    const int layer_num,
                                    const int ladder_num,
                                    const int module_num,
...
void Phase2PixelRawNtuple::fillEvt(const edm::Event& E) {
  evt_.run = E.id().run();
  evt_.evtnum = E.id().event();
}

void Phase2PixelRawNtuple::evt::init() {
  int dummy_int = 9999;
  run = dummy_int;
  evtnum = dummy_int;
}

void Phase2PixelRawNtuple::RecHit::init() {
...
std::pair<float, float> Phase2PixelRawNtuple::computeAnglesFromDetPosition(const SiPixelCluster& cl,
...
void Phase2PixelRawNtuple::processHits(const std::vector<std::pair<int, int> >& hitList) {
...
DEFINE_FWK_MODULE(Phase2PixelRawNtuple);
